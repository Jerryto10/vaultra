"""
Vaultra Admin Dashboard — Backend
===================================
Flask application for the Vaultra admin panel.
Manages clients, API keys, receipts, and metrics.
...
def seed_demo_data(db):
    """Insert demo clients and receipts if DB is empty."""
    count = db.execute("SELECT COUNT(*) FROM clients").fetchone()[0]
    if count > 0:
        return

    demo_clients = [
        ("Fintech Corp S.A.", "cto@fintech-corp.com", "starter", "active"),
        ("KreditBank GmbH", "compliance@kreditbank.de", "growth", "active"),
        ("PayFlow Europe", "tech@payflow.eu", "starter", "active"),
    ]

    client_ids = []
    for company, email, plan, status in demo_clients:
        cid = str(uuid.uuid4())
        ...
        db.execute("""
            ...
            (id, company_name, email, plan, status, api_key_hash, api_key_prefix,
             created_at, last_seen, receipt_count, monthly_limit)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (cid, company, email, plan, status, key_hash, key_prefix,
              time.time() - 86400*30, time.time() - 3600,
              1284 if company == "Fintech Corp S.A." else 342, limit))

    agents = ["credit-bot-v2", "kyc-agent-v1", "fraud-detector", "aml-scanner"]
    decisions = [
        ("REJECT loan #4821", "LOAN_REJECTED"),
        ("APPROVE loan #4820", "LOAN_APPROVED"),
    ...
    for i in range(50):
        ...
        """, (
            str(uuid.uuid4()), cid,
            random.choice(agents), dec, dec_type,
            hashlib.sha256(str(i).encode()).hexdigest()[:16],
            block + i,
...
def log_action(action, target=None, details=None):
    db = get_db()
    db.execute("""
        INSERT INTO activity_log (id, admin_id, action, target, details, ip_address, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
...
def create_client():
    ...
    try:
        db.execute("""
            INSERT INTO clients
            (id, company_name, email, plan, status, api_key_hash, api_key_prefix,
             created_at, receipt_count, monthly_limit)
            VALUES (?, ?, ?, ?, 'active', ?, ?, ?, 0, ?)
        """, (cid, company, email, plan, key_hash, key_prefix, time.time(), limit))
        db.commit()
        log_action("CREATE_CLIENT", cid, f"{company} / {email}")
...
def api_metrics():
    ...
    total_receipts = db.execute("SELECT COUNT(*) FROM receipts").fetchone()[0]
    flagged = db.execute("SELECT COUNT(*) FROM receipts WHERE status='review'").fetchone()[0]
    starter = db.execute("SELECT COUNT(*) FROM clients WHERE plan='starter' AND status='active'").fetchone()[0]
    growth = db.execute("SELECT COUNT(*) FROM clients WHERE plan='growth' AND status='active'").fetchone()[0]
    enterprise = db.execute("SELECT COUNT(*) FROM clients WHERE plan='enterprise' AND status='active'").fetchone()[0]
...
@app.route("/health")
def health():
    return jsonify({"status": "ok", "service": "vaultra-admin", "version": "1.0.0"})
