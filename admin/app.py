...
MAX_LOGIN_ATTEMPTS_PER_IDENTIFIER = 20
...
def check_login_rate(ip, identifier):
    ...
    per-IP cap (MAX_LOGIN_ATTEMPTS_PER_IP = 5) stays tight, since a single
    source hitting it is a strong single-origin brute-force/spray signal.
    The per-identifier cap (MAX_LOGIN_ATTEMPTS_PER_IDENTIFIER = 20) is
    higher, because check_login_rate runs before password verification and
    "admin"/"support" are guaranteed-existent, publicly-known seeded
    usernames: with a shared threshold of 5, an attacker with zero password
    knowledge could distribute 5 failed POSTs against "admin" across 5
    source IPs (each individually well under its own per-IP cap) and
    self-renew that lockout indefinitely once per LOGIN_WINDOW, denying the
    real admin service with negligible effort. Raising only the identifier
    cap (4x the IP cap) forces that same denial-of-service to cost an
    attacker meaningfully more — more source IPs and/or more requests per
    window — while leaving single-source brute-force/spray protection
    (the per-IP dimension) exactly as strong as before.
    """
    ...
    try:
        ...
        conn.execute("COMMIT")
        return ip_count < MAX_LOGIN_ATTEMPTS_PER_IP and id_count < MAX_LOGIN_ATTEMPTS_PER_IDENTIFIER
    finally:
        conn.close()
