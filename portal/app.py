...
def init_portal_db():
    ...
    # can't be created until after the ALTER TABLE migration runs. It backs
...
# ── CSV formula-injection defense (F8) ────────────────────────────────────
...
# exported file is opened. Several receipt fields (agent_id, decision_type,
# regulation) ultimately arrive via the public /api/receipt ingestion
# endpoint (admin/app.py) and are sanitized only for XSS, not for this — so
# any cell written to a CSV export must be neutralized at the writer
# boundary, regardless of whether that particular column is
# attacker-influenced today. Duplicated identically from admin/app.py: this
# codebase has no existing pattern for sharing code between admin/ and
# portal/, and a ~5-line pure function isn't worth introducing one for.
...
def check_login_rate(ip, identifier):
    ...
    anyone who knows or guesses an identifier (e.g. a leaked client email)
    could fill that identifier's bucket with 5 failed POSTs and lock the
    *legitimate* owner out for the whole LOGIN_WINDOW, even on a correct
    password; (2) with no per-IP counter at all, a single source could
    credential-spray across unlimited distinct accounts (a handful of
    guesses each) with no throttling whatsoever — exactly the class of
    attack ProxyFix's correct client-IP resolution was supposed to let this
    limiter catch.
...
def logout():
    ...
    # hook, F18): a GET route here let any third-party page log a victim out
...
def report_csv():
    ...
    for r in receipts:
        writer.writerow([csv_safe(v) for v in [
                        r["id"], r["agent_id"], r["decision_type"],
